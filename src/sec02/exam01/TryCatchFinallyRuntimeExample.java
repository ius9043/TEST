...
public class TryCatchFinallyRuntimeExample {
	...
	public TryCatchFinallyRuntimeExample() {
	...
	public void run() {
		...
		while (runFlag) {
			...
			System.out.println("1.물고기구매 2. 물고기판매 3.어항구매(" + bowlBuy + "원) 4.상태보기 5.먹이주기");
	...
	public void buyFish() {
		if (money < minFishBuy) {
			...
			for (int i = 0; i < fishName.length; i++) {
				System.out.println((i + 1) + "." + fishName[i] + "(구매가" + fishBuy[i] + "원, 판매가" + fishSell[i] + "원, 성장"
						+ fishGrow[i] + "일)");
			}
